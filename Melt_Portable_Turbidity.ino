...
bool ALSUpdate() {
  ...
  for (int i = 0; i < sensor.ALS.sensorCount ; i++) {
    ...
    for (int k = 0; k < ALS_AVE_COUNT; k++) {
      ...
      sensor.ALS.measure[i] += ads.readVoltage(1);    // Changed from sensor.ALS.measure[i] += ads.readVoltage(k+1); changed 12/04/23
